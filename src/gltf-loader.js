...
export function createGLTFLoader(manager) {
  ...
  return loader.register(gaussianSplatting).register(interactivity).register(extendedAttributes).register(materialVariants).register(nodeVisibility).register(animationPointer).register(()=>({name:'KHR_node_selectability'})).register(()=>({name:'KHR_node_hoverability'}));
}
export function configureDecoders(loader, renderer) {
  loader.setDRACOLoader(new DRACOLoader().setDecoderPath('/decoders/draco/').setWorkerLimit(2));
  loader.setMeshoptDecoder(MeshoptDecoder);
  loader.setKTX2Loader(new KTX2Loader().setTranscoderPath('/decoders/basis/').detectSupport(renderer).setWorkerLimit(2));
  return loader;
}
